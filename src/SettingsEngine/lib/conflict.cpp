    ...
    )
{
    ...
    if (RESOLUTION_LOCAL == rcChoice)
    {
        ...
        for (i = 0; i < cpProduct->rgdwValueCountLocal[dwValueIndex]; ++i)
        {
            ...
            hr = EnumWriteValue(pcdb, wzValueName, static_cast<CFG_ENUMERATION *>(cpProduct->rgcesValueEnumLocal[dwValueIndex]), i);
    ...
    else if (RESOLUTION_REMOTE == rcChoice)
    {
        ...
        {
            // TODO: In some cases this is writing to the main value when it's about to be overwritten again. Optimize?
            hr = EnumWriteValue(pcdb->pcdbLocal, wzValueName, static_cast<CFG_ENUMERATION *>(cpProduct->rgcesValueEnumRemote[dwValueIndex]), i);
